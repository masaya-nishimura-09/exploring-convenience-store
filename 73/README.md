...
| ---- | ---------------------------------------------------------------------------------- | ------------- | -------------- | ---------- | -------------------------------------------- | ------------------- |
| 73   | ATM → 飲み物、弁当、朝食、ペン、おやつをそれぞれ 2 つか 3 つのうちから 1 つずつ → レジ | 並列 Q 更新法 | ε = 0.01, Map1, similarity_threshold=0.7 | 写真で判別 | 最安値商品のみ購入。所持金不足なら購入しない | 同じ商品には同じ写真を使用。また、71の結果による最適類似度パラメータを使用。 |
