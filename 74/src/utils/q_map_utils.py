import os
from config_utils import get_item_amount, get_root_dir, get_vertical
from shopping_utils import item_picked_checker


# qマップを取得
def get_q_map():
    q_map = []
    for i in range(get_item_amount()):
        item_q = []
        for j in range(8):
            current_q = []
            with open(os.path.join(get_root_dir(), f"input/q/{i}/q{j}.txt")) as f:
                for _ in range(get_vertical()):
