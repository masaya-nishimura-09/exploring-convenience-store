...
int is_random_walk(double epsilon) {
    // int r = rand() % 10;
    double r = (double)rand() / RAND_MAX; // 0.0 ～ 1.0 random double number
    if (r < epsilon) {
        return 1;
    }
    else {
...
struct Position walk(
    ...
    if (random_walk == 0) {
        ...
        for (int i = 0; i < 4; i++) {
            if (value < iq[position.progress][position.x][position.y][i]) {
                value = iq[position.progress][position.x][position.y][i];
                next_direction = i;
            }
        }
    }
    ...
    if (next_x >= 0 && next_y >= 0 && next_x < vertical && next_y < horizontal){
        ...
        if (iq[position.progress][position.x][position.y][next_direction] > iq[position.progress][position.px][position.py][position.pd]) {
            iq[position.progress][position.px][position.py][position.pd] = iq[position.progress][position.x][position.y][next_direction] * 0.9;
        }
        ...
        if (store[next_x][next_y] == ' ' || store[next_x][next_y] == '*') {
            ...
            if (should_pick == 1) {
                ...
                iq[position.progress][position.x][position.y][next_direction] = 1;
            }
            return next_position;
        }
    } 
...
int main(void) {
    ...
    if ((epsilon_file = fopen("./input/epsilon.txt", "r")) == NULL) {
        ...
        fscanf(epsilon_file, "%lf", &epsilon);
    }
    fclose(epsilon_file); 
