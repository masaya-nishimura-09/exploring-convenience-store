...
def create_graph(x, y, ylabel, filename):
    ...
    plt.title(f"{TITLE}", fontsize=18, y=1.1)
...
with open('stats.csv', 'w', newline='', encoding='utf-8') as f:
    ...
    for d in range(3):
        ...
        for e in range(11):
            file_path = os.path.join(directory, f"{e}")
            name = f"file{DF_FILE}_map{d}_e{e}percent"
            output_data(file_path, name, csv_writer, d)
...
with open('stats.csv', newline='', encoding='utf-8') as f:
    ...
    for row in reader:
        if row['Map'] == '0':
            map0_titles.append(row['Title'])
            map0_mins.append(float(row['Min']))
            map0_maxs.append(float(row['Max']))
            map0_averages.append(float(row['Average']))
        elif row['Map'] == '1':
            map1_titles.append(row['Title'])
            map1_mins.append(float(row['Min']))
            map1_maxs.append(float(row['Max']))
            map1_averages.append(float(row['Average']))
        else:
            map2_titles.append(row['Title'])
            map2_mins.append(float(row['Min']))
            map2_maxs.append(float(row['Max']))
            map2_averages.append(float(row['Average']))

create_graph(epsilons, map0_mins, "Min Steps", f"./graph/file{DF_FILE}_map0_min_graph.png")
create_graph(epsilons, map0_maxs, "Max Steps", f"./graph/file{DF_FILE}_map0_max_graph.png")
create_graph(epsilons, map0_averages, "Average Steps", f"./graph/file{DF_FILE}_map0_average_graph.png")

create_graph(epsilons, map1_mins, "Min Steps", f"./graph/file{DF_FILE}_map1_min_graph.png")
create_graph(epsilons, map1_maxs, "Max Steps", f"./graph/file{DF_FILE}_map1_max_graph.png")
create_graph(epsilons, map1_averages, "Average Steps", f"./graph/file{DF_FILE}_map1_average_graph.png")

create_graph(epsilons, map2_mins, "Min Steps", f"./graph/file{DF_FILE}_map2_min_graph.png")
create_graph(epsilons, map2_maxs, "Max Steps", f"./graph/file{DF_FILE}_map2_max_graph.png")
create_graph(epsilons, map2_averages, "Average Steps", f"./graph/file{DF_FILE}_map2_average_graph.png")