...
| 61b  | ATM -> (x, y, z) -> レジ                                                           | 並列 Q 更新法 | ε = 0.01, Map1 | 写真で判別 | 最初に見つけた商品のみを購入する(一つ目の商品を見つけるとレジに行く)。レジで会計し、所持金が足りなければ買わない。 | 64   | atm -> water(lemon or plain) -> register                                           | 並列 Q 更新法 | ε = 0.01, Map1 | 写真で判別 | カテゴリを追加。prefer_lower_price_items = ture で実行                                                            |
