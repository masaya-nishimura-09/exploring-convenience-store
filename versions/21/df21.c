...
struct Position walk(char store[vertical][horizontal], char shopping_list[max_items], char cart[max_items], FILE *file, float q[vertical][horizontal][4], struct Position position) {
    ...
    if (next_x >= 0 && next_y >= 0 && next_x < vertical && next_y < horizontal && store[next_x][next_y] != ' '){
        if (store[next_x][next_y] == '0' || store[next_x][next_y] == '1') {
            ...
            if (position.q_state == 1) {
                next_position.q_span = position.q_span - 1;
                if (next_position.q_span < 0) {
                    next_position.q_span = 20;
...
int main(void) {
    ...
    FILE *file;
    file = fopen(now, "w");
    fprintf(file, "df21.c output data\n\n");
    ...
    char *item_urls[4] = { "./input/q/item_q/iq0.txt", "./input/q/item_q/iq1.txt", "./input/q/item_q/iq2.txt", "./input/q/item_q/iq3.txt" };
    char *shelf_urls[4] = { "./input/q/shelf_q/sq0.txt", "./input/q/shelf_q/sq1.txt", "./input/q/shelf_q/sq2.txt", "./input/q/shelf_q/sq3.txt" };
    read_q(iq, item_urls); 
    read_q(sq, shelf_urls); 
    ...
    while(keep_going == 1) {
        if (position.q_state == 0) {
            position = walk(store, shopping_list, cart, file, sq, position);
        }
        else {
            position = walk(store, shopping_list, cart, file, iq, position);
        }

        if (position.q_state == 1 && position.q_span == 20) {
            position.q_state = 0;
        }
    ...
    write_q(iq, item_urls);
    write_q(sq, shelf_urls);
