...
def item_checker(status, item_code):
    ...
    # レジ以外を全て見つけた　かつ　見つけたものがレジでない時
    if status.progress == ITEM_AMOUNT - 1 and not are_same_list(item_rgb, cashier_rgb):
        return False
    ...
    if next_x >= 0 and next_y >= 0 and next_x < VERTICAL and next_y < HORIZONTAL:
        ...
        # 次の場所に商品がある時
            ...
            if should_pick:
                ...
                # 進捗を更新
                # 今回は一つ目の商品を見つけたらレジを探す
                if status.progress == 0:
                    status.progress = status.progress + 1
                elif status.progress == ITEM_AMOUNT - 1:
                    status.progress = status.progress + 1
                else:
                    status.progress = ITEM_AMOUNT - 1
