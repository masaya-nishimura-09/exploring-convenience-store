...
int q_comparison(
    ...
    int next_direction = random_direction;
    float value = 0;
...
struct Position walk(
    ...
    if (random_walk == 0) {
        next_direction = q_comparison(iq, position.x, position.y, next_direction, position.progress, shopping_list, cart);
    }
