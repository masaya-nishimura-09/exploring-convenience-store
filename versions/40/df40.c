...
struct Position walk(
    ...
    if (next_x >= 0 && next_y >= 0 && next_x < vertical && next_y < horizontal){
        ...
        if (store[next_x][next_y] == ' ' || store[next_x][next_y] == '*') {
            ...
            if (iq[q_number][position.x][position.y][next_direction] > iq[q_number][position.px][position.py][position.pd]) {
                iq[q_number][position.px][position.py][position.pd] = iq[q_number][position.x][position.y][next_direction] * 0.9;
            }
...
int main(void) {
    ...
    FILE *file;
    file = fopen(now, "w");
    fprintf(file, "df40.c output data\n\n");
