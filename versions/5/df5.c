...
void dfs(int v, int n, int dfnum[], int vv[][2], int o, int ss[][2], int ee[][2], int matrix[n][n]) {
    printf("v%dを訪問\n", v + 1);
    dfnum[v] = 1;
    double shortest = 1e9;
    int next = 0;
    ...
    if (next == 0) {
        ...
        dfs(next, n, dfnum, vv, o, ss, ee, matrix);
    }
}
...
int main(void) {
    ...
    dfs(0, n, dfnum, vv, o, ss, ee, matrix);
