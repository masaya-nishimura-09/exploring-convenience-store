...
struct Q {
    float s1q[vertical][horizontal][4];
    float s2q[vertical][horizontal][4];
    float s3q[vertical][horizontal][4];
    float i1q[vertical][horizontal][4];
    float i2q[vertical][horizontal][4];
    float i3q[vertical][horizontal][4];
};
...
struct Position walk(char store[vertical][horizontal], char shopping_list[max_items], char cart[max_items], FILE *file, float q[vertical][horizontal][4], struct Position position) {
    ...
    if (next_x >= 0 && next_y >= 0 && next_x < vertical && next_y < horizontal && store[next_x][next_y] != ' '){
        if (store[next_x][next_y] == '0' || store[next_x][next_y] == '1') {
            ...
            next_position.q_state = position.q_state;
            ...
            next_position.q_state = 1;

            if (position.q_state == 0) {
        ...
        next_position.progress = position.progress;
        next_position.q_state = position.q_state;
        next_position.q_span = position.q_span;
        return next_position;
    }
}
...
int main(void) {
    ...
    FILE *file;
    file = fopen(now, "w");
    fprintf(file, "df22.c output data\n\n");
    ...
    char *urls_1[4] = { "./input/q/item/i1q/i1q_0.txt", "./input/q/item/i1q/i1q_1.txt", "./input/q/item/i1q/i1q_2.txt", "./input/q/item/i1q/i1q_3.txt" };
    char *urls_2[4] = { "./input/q/item/i2q/i2q_0.txt", "./input/q/item/i2q/i2q_1.txt", "./input/q/item/i2q/i2q_2.txt", "./input/q/item/i2q/i2q_3.txt" };
    char *urls_3[4] = { "./input/q/item/i3q/i3q_0.txt", "./input/q/item/i3q/i3q_1.txt", "./input/q/item/i3q/i3q_2.txt", "./input/q/item/i3q/i3q_3.txt" };
    char *urls_4[4] = { "./input/q/shelf/s1q/s1q_0.txt", "./input/q/shelf/s1q/s1q_1.txt", "./input/q/shelf/s1q/s1q_2.txt", "./input/q/shelf/s1q/s1q_3.txt" };
    char *urls_5[4] = { "./input/q/shelf/s2q/s2q_0.txt", "./input/q/shelf/s2q/s2q_1.txt", "./input/q/shelf/s2q/s2q_2.txt", "./input/q/shelf/s2q/s2q_3.txt" };
    char *urls_6[4] = { "./input/q/shelf/s3q/s3q_0.txt", "./input/q/shelf/s3q/s3q_1.txt", "./input/q/shelf/s3q/s3q_2.txt", "./input/q/shelf/s3q/s3q_3.txt" };
    ...
    while(keep_going == 1) {
        ...
        if (position.q_state == 0) {
            if (position.progress == 0) {
                position = walk(store, shopping_list, cart, file, q.s1q, position);
            }
            else if (position.progress == 1) {
                position = walk(store, shopping_list, cart, file, q.s2q, position);
            }
            else {
                position = walk(store, shopping_list, cart, file, q.s3q, position);
            }
        }
        else {
            if (position.progress == 0) {
                position = walk(store, shopping_list, cart, file, q.i1q, position);
            }
            else if (position.progress == 1) {
                position = walk(store, shopping_list, cart, file, q.i2q, position);
            }
            else {
                position = walk(store, shopping_list, cart, file, q.i3q, position);
            }
        }
    ...
    write_q(q.i1q, urls_1);
    write_q(q.i2q, urls_2);
    write_q(q.i3q, urls_3);
    write_q(q.s1q, urls_4);
    write_q(q.s2q, urls_5);
    write_q(q.s3q, urls_6);
