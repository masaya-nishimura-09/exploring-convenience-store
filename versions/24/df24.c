...
struct Position walk(char store[vertical][horizontal], char shopping_list[max_items], char cart[max_items], FILE *file, float sq[vertical][horizontal][4], float iq[vertical][horizontal][4], float cq[vertical][horizontal][4], struct Position position) {
    ...
    if (next_x >= 0 && next_y >= 0 && next_x < vertical && next_y < horizontal && store[next_x][next_y] != ' '){
        if (store[next_x][next_y] == '0' || store[next_x][next_y] == '1') {
            ...
            if (position.progress == 0 || position.progress == 1) {
...
int main(void) {
    ...
    FILE *file;
    file = fopen(now, "w");
    fprintf(file, "df24.c output data\n\n");
